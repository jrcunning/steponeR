...
steponeR <- function(files, delim=",", target.ratios, fluor.norm,
  ...
  if("STANDARD" %in% tasks) {
    ...
    if("UNKNOWN" %in% tasks) {
      ...
      copymeans <- reshape2::dcast(unk, Sample.Plate ~ Target.Name, mean, na.rm=T, value.var="copies")
  ...
  if(nrow(unk)!=0) {
    ctmeans <- reshape2::dcast(unk, Sample.Plate ~ Target.Name, mean, na.rm=T, value.var="CT")  # na.rm=F
    colnames(ctmeans) <- c(colnames(ctmeans)[1], paste(colnames(ctmeans)[-1], "CT.mean", sep="."))
    ctsds <- reshape2::dcast(unk, Sample.Plate ~ Target.Name, sd, na.rm=T, value.var="CT")
    colnames(ctsds) <- c(colnames(ctsds)[1], paste(colnames(ctsds)[-1], "CT.sd", sep="."))
    techreps <- reshape2::dcast(unk, Sample.Plate ~ Target.Name, value.var="CT",
